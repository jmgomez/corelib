...
var Route = /** @class */ (function () {
    function Route(repo) {
        ...
        this.getCountBy = function (req, res) {
            var query = req.query;
            _this.repo.countBy(RepositoryQuery_1.RepositoryQuery.fromQueryStringTo(query, _this.repo))
                .subscribe(NodeUtils_1.NodeUtils.writeResponse(res), NodeUtils_1.NodeUtils.writeError(res));
        };
        this.getRangeBy = function (req, res) {
            var query = req.query;
            _this.repo.getRangeBy(RepositoryQuery_1.RepositoryQuery.fromQueryStringTo(query, _this.repo), parseInt(query.skip), parseInt(query.limit))
                .subscribe(NodeUtils_1.NodeUtils.writeResponse(res), NodeUtils_1.NodeUtils.writeError(res));
        };
    ...
    Route.prototype.configureRouter = function () {
        ...
        router.route("/countby").get(this.getCountBy).options(NodeUtils_1.NodeUtils.okOptions);
        router.route("/rangeby").get(this.getRangeBy).options(NodeUtils_1.NodeUtils.okOptions);
